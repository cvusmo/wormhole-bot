...
impl EventHandler for Handler {
    ...
    if msg.content == "!rss" || true {
        ...
        //if let Err(e) = self.check_rss(&ctx).await {
            //println!("Error checking RSS: {:?}", e);
        //}
...
impl Handler {
    async fn check_rss(&mut self, ctx: &Context) -> Result<(), Box<dyn Error>> {
        let content = reqwest::get("https://status.robertsspaceindustries.com/index.xml")
            .await?
            .bytes()
            .await?;    let channel = Channel::read_from(&content[..])?;
    ...
    if let Some(entry) = latest_entry {
        let entry_id = entry.guid().map(|g| g.value().to_string())
            .unwrap_or_else(|| {
                println!("Warning: No GUID, using default ID");
        ...
        if self.last_entry_id.as_ref().map_or(true, |id| id != &entry_id) {
            ...
            let message = format!(
                "**{}**\n*Published: {}*\n{}\n{}",
                title, pub_date, description, link
            );
        ...
        println!("No entries in RSS feed");
    }
    Ok(())
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    // Retrieve DISCORD_TOKEN and CHANNEL_ID
