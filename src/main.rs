...
async fn main() -> Result<(), Box<dyn Error>> {
    // Initialize log
    logger::init_logger();
    logger_log_info("Starting wormhole-rss...");
    ...
    let mut client = Client::builder(&discord_token, GatewayIntents::GUILD_MESSAGES, GatewayIntents::MESSAGE_CONTENT).event_handler(handler::Handler {
    ...
    if let Err(whye) = client.start().await {
        logger::log_error(&format!("Client error: {:?}", whye));
    }
